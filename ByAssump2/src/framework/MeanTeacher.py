...
class MeanTeacherDenoise():
    def __init__(self,
        ...
        self.Round0_noisy_data_path = self.indexlization(target_set=self.noisy_pos_set,
                                                         file_suffix=self.name + '0noisy_train.txt')
        ...
        for loop_iter in range(2, self.loop_times + 1):
            ...
            if mode == 0:
                ...
                with open(self.Round0_noisy_data_path, 'r') as f:
                    for line in f:
                        if self.trustable_count[eval(line)['id']] < 3:
                            ...
                            trusted.append(line)
                noisy_data_path = self.tmp_data_saving(data=leftout,
                                                       suffix=self.name + str(loop_iter) + "noisy_train.txt")
                ...
                torch.save(self.teacher_model.state_dict(),
                           open(os.path.join(self.workspace_path, "teacher" + str(loop_iter) + ".model"), "wb"))
                torch.save(self.student_model.state_dict(),
                           open(os.path.join(self.workspace_path, "student" + str(loop_iter) + ".model"), "wb"))
                ...
                self.IF_calc_step1(input_file=train_data_path, batch_size=self.batch_size_identifier(ins_num, 40),
                                   model_path=self.workspace_path + "/student" + str(loop_iter) + ".model", scale=scale,
                                   damp=damp)
                self.IF_calc_step2(input_file=train_data_path, input_file2=noisy_data_path,
                                   model_path=self.workspace_path + "/student" + str(loop_iter) + ".model", scale=scale,
                                   damp=damp)
                selected_clean, selected_noisy = self.get_results(
                    IF_value_matrix=self.workspace_path + "/" + self.name + str(
                        loop_iter) + "clean_res_dir/IF_value_result.npy",
                    clean_set_path=train_data_path, noisy_set_path=noisy_data_path)
            elif mode == 1:
                candidate_pos, candidate_NA, leftout, trusted = self.selected_data_construction(selected_clean,
                                                                                                selected_noisy)
                train_data_path = self.train_data_construction(self.clean_pos_set, trusted, [], loop_iter)
                noisy_data_path = self.tmp_data_saving(data=leftout,
                                                       suffix=self.name + str(loop_iter) + "noisy_train.txt")
                ...
                noisy_data_path = self.tmp_data_saving(data=leftout,
                                                       suffix=self.name + str(loop_iter) + "noisy_train.txt")
                if loop_iter < 3:
                    self.overfitting_parameters_with_ema(train_data_path=train_data_path,
                                                         target_model=self.student_model, ema_model=self.teacher_model,
                                                         gamma=0.0)
                    self.update_ema_variables(model=self.student_model, ema_model=self.teacher_model, step=loop_iter,
                                              alpha=0.01)
                else:
                    self.overfitting_parameters_with_ema(train_data_path=train_data_path,
                                                         target_model=self.student_model, ema_model=self.teacher_model,
    ...
    def init_train(self):
        ...
        scale, damp = IFcf.process()
        self.IF_calc_step1(input_file=Round0_train_data_path, batch_size=self.batch_size_identifier(ins_num, 40),
        ...
        return self.get_results(
            IF_value_matrix=self.workspace_path + "/" + self.name + "0clean_res_dir/IF_value_result.npy",
    ...
    def IF_calc_step1(self, input_file, batch_size, model_path, scale, damp):
        ...
        a = subprocess.Popen(
            ["./run3_step1.sh", input_file, '%d' % (self.line_counter(file=input_file) // 4 + 1), '%d' % batch_size,
    ...
    def IF_calc_step2(self, input_file, input_file2, model_path, scale, damp):
        ...
        a = subprocess.Popen(
            ["./run3_step2.sh", input_file, '%d' % (self.line_counter(file=input_file) // 2 + 1), input_file2,
             model_path, '%d' % scale, '%f' % damp, '%s' % self.config_file], shell=False)
        while a.poll() is None:
            time.sleep(1)
        return a
