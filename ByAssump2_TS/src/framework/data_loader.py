...
class SentenceREDataset(data.Dataset):
    ...
    def __init__(self, path, rel2id, tokenizer, kwargs):
        ...
        f.close()
        print("Loaded sentence RE dataset {} with {} lines and {} relations.".format(path, len(self.data), len(self.rel2id)))
    ...
    def eval(self, pred_result, use_name=False, binary=False):
        """
        Args:
        ...
        for idx, i in enumerate(range(total)):
            ...
            if golden == pred_result[i]:
                ...
                self.mistaken_classification.append(idx)
            if golden != neg:
                gold_positive +=1
        ...
        try:
            ...
            micro_f1 = 0
        result = {'acc': acc,'micro_f1':micro_f1, 'micro_p':micro_p, 'micro_r':micro_r}
...
class BagREDataset(data.Dataset):
    ...
    def __init__(self, path, rel2id, tokenizer, entpair_as_bag=False, bag_size=0, mode=None):
        ...
        # Construct sentence-level evaluation set
            if mode == "Sent" :
                self.weight = np.ones((len(self.rel2id)), dtype=np.float32)
                self.bag_scope = []
                self.name2id = {}
    ...
    def collate_fn(data):
        ...
        for i in range(len(seqs)):
            ...
            seqs[i] = seqs[i].expand((torch.cuda.device_count() if torch.cuda.device_count() > 0 else 1, ) + seqs[i].size()).clone()
        scope = [] # (B, 2)
        start = 0
        for c in count:
    ...
    def eval(self, pred_result):
        ...
        if self.mode == 'Sent' :
            ...
            auc = sklearn.metrics.auc(x=rec[:2000], y=prec[:2000])
            np_prec = np.array(prec)
            np_rec = np.array(rec) 
            f1 = (2 * np_prec * np_rec / (np_prec + np_rec + 1e-20)).max()
            mean_prec = np_prec.mean()
            result = {'prec': np_prec, 'rec': np_rec, 'mean_prec': mean_prec, 
            'f1': f1, 'auc': auc, 'Top100' : prec[100], 'Top200' : prec[200],
            'Top500' : prec[500]}
        return result

def BagRELoader(path, rel2id, tokenizer, batch_size, 
        shuffle, mode=None, entpair_as_bag=False, bag_size=0, num_workers=2, 
        collate_fn=BagREDataset.collate_fn):
    if bag_size == 0:
        collate_fn = BagREDataset.collate_fn
    else:
        collate_fn = BagREDataset.collate_bag_size_fn
    dataset = BagREDataset(path, rel2id, tokenizer, entpair_as_bag=entpair_as_bag, bag_size=bag_size, mode=mode)
    data_loader = data.DataLoader(dataset=dataset,
            batch_size=batch_size,
            shuffle=shuffle,
            num_workers=num_workers,
