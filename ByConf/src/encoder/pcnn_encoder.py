...
class PCNNEncoder(BaseEncoder):
    ...
    def __init__(self, 
        ...
        super().__init__(token2id, max_length, hidden_size, word_size, 
                position_size, blank_padding, word2vec, mask_entity=mask_entity, fix_embedding=fix_embedding)
        if entityTyping :
            self.input_size = self.word_size + 2*position_size + 50
        self.drop = nn.Dropout(dropout)
        self.kernel_size = kernel_size
        self.padding_size = padding_size
        self.act = activation_function
        self.entityTyping = entityTyping
        ...
        if self.entityTyping :
            self.entity_embedding = nn.Embedding(4, 50)
        self._minus = -100

        self.hidden_size *= 3
    def forward(self, token, pos1, pos2, mask, entity_type=None):
        """
        Args:
        ...
        if self.entityTyping :
            x = torch.cat([self.word_embedding(token),
                           ...
                           self.entity_embedding(entity_type)], 2)
        else :
            x = torch.cat([self.word_embedding(token), 
                        self.pos1_embedding(pos1), 
    ...
    def tokenize(self, item):
        ...
        pos_tail = item['t']['pos']
        if self.entityTyping :
            head_type = item['h']['type']
            tail_type = item['t']['type']
        # Sentence -> token
        if not is_token:
        ...
        for idx in range(len(pos1)) :
            pos1[idx] = max(90, pos1[idx])
            pos2[idx] = max(90, pos2[idx])
            pos1[idx] = min(150, pos1[idx])
            pos2[idx] = min(150, pos2[idx])
        '''
        ...
        if self.entityTyping :
            type_list = []
            for idx, t in enumerate(range(self.max_length)) :
                if idx >= pos_head[0] and idx < pos_head[1] :
                    type_list.append(self.type2id[head_type])
                elif idx >= pos_tail[0] and idx < pos_tail[1] :
                    type_list.append(self.type2id[tail_type])
                else :
                    type_list.append(self.type2id["NA"])
            type_list = torch.tensor(type_list).long().unsqueeze(0)
            return indexed_tokens, pos1, pos2, mask, type_list
        else :
